...
//    virtual CTerrainNode *GetLeafNodeAt(Meter x, Meter y)=0;
    ...
    //stats
    virtual const int GetTerrainSize()const = 0;
    virtual const int GetSectorSize()const = 0;
    virtual const int GetHeightMapUnitSize()=0;
...
//    virtual void SetNodePyramid(int treeLevel, int x, int y, CTerrainNode *node)=0;
    ...
    //
    virtual bool Load(AZ::IO::HandleType fileHandle, int nDataSize, STerrainChunkHeader* pTerrainChunkHeader, STerrainInfo* pTerrainInfo, std::vector<struct IStatObj*>** ppStatObjTable, std::vector<_smart_ptr<IMaterial> >** ppMatTable)=0;
    virtual bool ITerrain::Load(const char *, STerrainInfo *)=0;
};
