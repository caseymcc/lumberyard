...
public:
    ...
    {
        return (m_queuedJobs.Find(node)!=-1||m_runningJobs.Find_If([&](Dispatch<_NodeType> &value){return (value.node==node); })!=-1);
    };
...
private:
    bool RunJob(_NodeType *node, bool executeAsJob, const SRenderingPassInfo &passInfo);
