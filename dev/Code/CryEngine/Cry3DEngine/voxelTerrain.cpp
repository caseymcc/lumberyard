...
# else
    ...
    pTerrainInfo->type=STerrainInfo::Voxel;
...
const int VoxelTerrain::GetTerrainSize() const
{
    return 0;
}

const int VoxelTerrain::GetSectorSize() const
{
    return 0;
}
...
const int VoxelTerrain::GetHeightMapUnitSize()
{
    return 1;
}
...
bool VoxelTerrain::Load(AZ::IO::HandleType fileHandle, int dataSize, STerrainChunkHeader* chunkHeader, STerrainInfo* terrainInfo, std::vector<struct IStatObj*>** staticObjectTable, std::vector<_smart_ptr<IMaterial> >** materialTable)
{
...
{
    std::vector<voxigen::Key> updatedChunks=m_world.getUpdatedChunks();
