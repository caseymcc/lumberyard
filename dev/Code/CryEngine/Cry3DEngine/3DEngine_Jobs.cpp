...
void C3DEngine::CreateRNTmpData(CRNTmpData** ppInfo, IRenderNode* pRNode, const SRenderingPassInfo& passInfo)
{
    // g_cCheckCreateRNTmpData lock scope
    {
        ...
        assert(!pElem->pOwnerRef || !(*pElem->pOwnerRef) || !(*(pElem->pOwnerRef))->userData.m_pFoliage);
...
void C3DEngine::RenderRenderNode_ShadowPass(IShadowCaster* pShadowCaster, const SRenderingPassInfo& passInfo, AZ::LegacyJobExecutor* pJobExecutor)
{
    ...
    case eERType_Vegetation:
    {
        ...
        pVegetation->SetDrawFrame(passInfo.GetFrameID(), passInfo.GetRecursiveLevel());
        const CLodValue lodValue = pVegetation->ComputeLod(wantedLod, passInfo);
        pVegetation->Render(passInfo, lodValue, NULL, rendItemSorter);
    }
    break;
    case eERType_Brush:
    {
...
void CVegetation::UpdateRndFlags()
{
    ...
    const uint32 dwFlagsToUpdate =
        ERF_CASTSHADOWMAPS | ERF_DYNAMIC_DISTANCESHADOWS | ERF_HIDABLE
    ...
    IRenderNode::SetViewDistanceMultiplier(vegetGroup.fMaxViewDistRatio);
}
